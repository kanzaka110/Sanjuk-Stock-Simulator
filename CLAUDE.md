...
/야간               →  야간 프리브리핑 (내일 한국장 지정가 예약 주문용, $0)
/미국장             →  미국장 종합 브리핑 (뉴스 교차검증 + RIA + 10단계 분석, $0)
/미국장-야간         →  미국장 프리브리핑 (오늘 밤 지정가 예약 주문용, $0)
...
도움말            →  명령어 목록
(브리핑은 CLI 스케줄 트리거로 자동 전송 — 봇에서 직접 요청 불가)
```
...
BRIEFING_TYPE=KR_NIGHT   →  야간 프리브리핑 (한국 종목 + 지정가 예약 주문 + 갭 시나리오)
BRIEFING_TYPE=US_NIGHT   →  미국장 프리브리핑 (미국 종목 + 지정가 예약 주문 + 선물 시나리오)
