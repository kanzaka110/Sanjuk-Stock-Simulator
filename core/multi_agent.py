...
- **진입가는 범위가 아닌 조건부 지정가**: "₩57,500~58,500 (갭다운 시 ₩56,000)" 식으로 시나리오별 가격
- **주문 유효 시간 명시**: "내일 09:00~09:30 시초가 매수" 또는 "종일 지정가" 등
- **갭 시나리오 분석 필수**: 미국장 결과 기반으로 내일 갭업/갭다운/보합 시나리오별 대응
  · 갭업 시: 추격 매수 OK인지, 지정가를 올려야 하는지
  · 갭다운 시: 더 낮은 지정가로 대기할지, 추가 매수 기회인지
  · 보합 시: 기본 지정가 유지
- **"내일 아침 예약 주문 요약"** 섹션을 반드시 포함 — MTS/HTS에 바로 입력 가능한 형태:
  · 종목명 | 계좌 | 지정가 | 수량 | 유효시간 | 조건
- 가격이 이미 올라서 매수 타이밍을 놓친 경우 → 다음 눌림목 가격대를 제시하고 "대기" 판단"""
...
━━━ 💵 지정가 주문 규칙 ━━━
- **모든 매수 추천에 지정가 필수**: "진입가 $185.00 이하" 형식으로 구체적 가격 제시
- **진입가는 조건부 지정가**: "$183.50~185.00 (프리마켓 약세 시 $180.00)" 식으로 시나리오별 가격
...
- **진입가는 조건부 지정가**: "$183.50~185.00 (프리마켓 약세 시 $180.00)" 식으로 시나리오별 가격
- **주문 유효 기간 명시**: "당일 지정가(DAY)" 또는 "GTC(취소 전 유효)" 등
- **프리마켓/선물 분석 필수**: 선물 동향 기반 개장 시나리오별 대응
  · 선물 상승 시: 추격 매수 OK인지, 지정가를 올려야 하는지
  · 선물 하락 시: 더 낮은 지정가로 대기할지, 추가 매수 기회인지
  · 선물 보합 시: 기본 지정가 유지
- **"오늘 밤 지정가 주문 요약"** 섹션을 반드시 포함 — HTS에 바로 입력 가능한 형태:
  · 종목명 | 계좌 | 지정가 | 수량 | 유효기간 | 조건
- 가격이 이미 올라서 매수 타이밍을 놓친 경우 → 다음 눌림목 가격대를 제시하고 "대기" 판단
- 개장 전 2.5시간 여유가 있으므로 → **주문 넣고 자도 되는지** 명시 (밤새 모니터링 불필요 여부)"""
    else:
        market_focus = ""
...
④ 아부 금지. 데이터 기반 직언.
⑤ 모든 수치는 구체적으로 (%, 가격)
⑧ AI 메모리의 [신뢰도 기반 판단 보정]을 반드시 반영 — 🔴 위험 종목은 샘플 수에 따라 -15~-30% 감점, 🟢 고신뢰 종목은 +5~+10% 가중 (시스템이 자동 보정). 위험 종목 추천 시 과거 실패 이유를 명시.
  ...
  "night_orders": [
    {
      ...
      "조건": "갭다운 시 ₩56,000으로 변경"
    }
  ],
  "gap_scenarios": {
    "갭업_1pct_이상": "추격 매수 여부 + 조정된 지정가",
    "보합": "기본 지정가 유지 여부",
    "갭다운_1pct_이상": "추가 매수 기회 + 하향 지정가"
  }"""
  ...
  "night_orders": [
    {
      ...
      "수량": "5주",
      "유효기간": "DAY 또는 GTC",
      "조건": "프리마켓 약세 시 $180.00으로 변경"
    }
  ],
  "gap_scenarios": {
    "선물_상승": "추격 매수 여부 + 조정된 지정가",
    "선물_보합": "기본 지정가 유지 여부",
    "선물_하락": "추가 매수 기회 + 하향 지정가"
  }"""
    else:
        night_orders_schema = ""
