...
- 개장 전 2.5시간 여유가 있으므로 → **주문 넣고 자도 되는지** 명시"""
        ...
        market_focus = f"""
⑥ 이 브리핑은 【데일리 리뷰】입니다 (KST 07:00). 지난 하루의 한국장+미국장 **양 시장 전체**를 결산합니다.
⑦ 구성 순서 (반드시 이 순서로):
   1. 어제 결산 — 한국장 마감(코스피/코스닥 + 보유 한국 종목 등락) + 밤새 미국장 마감(지수 + 보유 미국 종목 등락)
   2. 📒 내 액션 리뷰 — 컨텍스트의 "어제의 매매 리뷰"를 기반으로 사용자가 실행한 매매를 평가:
      · 각 매매의 실현손익(금액+%)을 명시하고 잘한 점/아쉬운 점을 직언
      · "어제 종료된 추천" 결과(승/패)도 함께 — AI 추천의 적중 여부 솔직히 결산
      · 매매 기록이 없으면 "어제 실행한 매매 없음" + 어제 브리핑 액션 대비 미실행 항목 점검
   3. 보유 조치 — 전 계좌(한국+미국) 잔여 포지션의 오늘 조치 (홀딩/익절/손절, 시계별)
   4. 오늘의 액션 — 한국장 개장(09:00) 전 실행할 것이 있으면 actions에 (없으면 빈 배열)
{ria_focus}

━━━ 🌅 데일리 리뷰 규칙 ━━━
- 어제 평가액 변화를 수치로: "전체 평가 N만 → M만 (±X만, ±Y%)"가 가능하면 제시
- 내 액션 리뷰는 비판 포함 직언 — 아부 금지. 단 결과론적 비난 금지 (당시 정보 기준 판단의 적절성 평가)
- 보유 조치는 시계 규칙 적용: 〔장기〕는 논지 점검만, 〔단기〕는 익절/손절 라인 갱신
- 상세 신규 분석은 08:50 KR_OPEN과 20:00 US_NIGHT에서 — 여기선 결산+조치 중심"""
    else:
        market_focus = ""
