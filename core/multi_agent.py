...
- 개장 전 2.5시간 여유가 있으므로 → **주문 넣고 자도 되는지** 명시"""
        ...
        market_focus = f"""
⑥ 이 브리핑은 【데일리 리뷰】입니다 (KST 07:00). **결산·복기 전용 브리핑** — 지난 하루의 한국장+미국장 양 시장을 돌아보고 현재 내 포지션 상태를 점검한다.
⑦ ★★ 절대 규칙: 이 브리핑은 신규 매수/매도를 권고하지 않는다. actions는 **반드시 빈 배열 []**. ★★
   - 신규 진입/청산 판단은 08:50 KR_OPEN(개장 직전)과 20:00 US_NIGHT(미국장 전)가 전담한다.
   - 여기서 매수/매도 추천을 내면 1시간 50분 뒤 KR_OPEN과 중복되어 사용자를 혼란시킨다.
   - 보유 종목에 "당장 손절선 이탈" 같은 긴급 상황이 보이면 → actions가 아니라 본문에 "⚠️ KR_OPEN에서 점검 요망"으로만 기록.
⑧ 구성 순서 (반드시 이 순서로, 결산 3부):
   1. 📊 어제 시장 결산 — 한국장 마감(코스피/코스닥 + 보유 한국 종목 등락) + 밤새 미국장 마감(지수 + 보유 미국 종목 등락)
   2. 📒 내 거래 복기 — 컨텍스트의 "어제의 매매 리뷰"를 기반으로 사용자가 실행한 매매를 평가:
      · 각 매매의 실현손익(금액+%)을 명시하고 잘한 점/아쉬운 점을 직언
      · "어제 종료된 추천" 결과(승/패)도 함께 — AI 추천의 적중 여부 솔직히 결산
      · 매매 기록이 없으면 "어제 실행한 매매 없음"
   3. 💼 현재 포지션 상태 — 전 계좌 보유 종목의 현황 점검 (시계별 논지 유효성, 손익 상태). 단 "사라/팔아라"가 아니라 "현재 이런 상태다"라는 진단만.
{ria_focus}

━━━ 🌅 데일리 리뷰 규칙 ━━━
- actions = [] 고정. investment_decision은 "관망"으로. strategy_buy/strategy_sell도 빈 배열.
- 어제 평가액 변화를 수치로: "전체 평가 N만 → M만 (±X만, ±Y%)"가 가능하면 제시
- 내 거래 복기는 비판 포함 직언 — 아부 금지. 단 결과론적 비난 금지 (당시 정보 기준 판단의 적절성 평가)
- 포지션 점검은 시계 규칙: 〔장기〕는 논지 유효성만, 〔단기/중기〕는 "손익 상태 + 다음 분기점이 무엇인지" 진단만 (실행 지시 금지)
- 긴급 사안은 "KR_OPEN에서 확인 요망"으로 넘긴다 — 여기서 실행시키지 않는다
