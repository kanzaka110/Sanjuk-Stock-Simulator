    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "confidence": {
                ...
                "minimum": 0,
                "maximum": 100,
                "description": "확신도 (0-100). 50~55 도피 금지 — 근거 조건 수에 비례해 30~90 전 범위 사용. 강한 합류면 75+, 근거 약하면 35 이하로 과감히.",
            },
            "reasoning": {
                "type": "string",
...
- 가격이 이미 올라서 매수 타이밍을 놓친 경우 → 분석에 "다음 눌림목 대기" 판단, night_orders는 빈 배열"""
    elif briefing_type == "KR_OPEN":
        market_focus = """
⑥ 이 브리핑은 【한국장 개장 직전 최종 액션 브리핑】입니다 (KST 08:50 발송, 09:00 개장 10분 전).
⑦ 목적: 사용자가 09:00~10:00에 실행할 액션을 **최종 확정**하는 것. 분석보다 결론 — actions가 전부다.

━━━ 🔔 개장 직전 특화 규칙 ━━━

① **밤새 미국장 마감 결과 반영 필수**: 지수 등락·보유 미국주(MU/LMT) 결과 → 오늘 한국장 갭 방향 판단.
② **동시호가(08:30~09:00) 갭 시나리오 — 시초가 기준 행동 분기 필수**:
   · 갭업 +1.5% 초과: 추격 금지 — 매수 액션 취소조건 발동, 눌림목 재설정
   · 보합 ±1.5%: 기존 액션 그대로 실행
   · 갭다운 -1.5% 초과: 지정가 하향 조정 (구체 가격 제시) 또는 첫 30분 관망 후 진입
③ **어제 야간 프리브리핑(KR_NIGHT)의 예약 주문 재검토**: 밤새 상황 변화로 무효화된 주문은 "취소하라"고 명시, 유효한 주문은 "유지"로 확인.
④ 09:00~09:30 시초가 변동성 구간 주의 — 시장가 주문 금지, 전부 지정가.
⑤ actions의 type은 "매수·즉시/매도·즉시"(개장 직후 입력) 위주. 조건 대기는 취소조건/대기 트리거로.
⑥ 간결하게 — 상세 분석은 어제 야간 브리핑에 있음. 이 브리핑은 "오늘 아침 뭘 하나"만."""
...
- 개장 전 2.5시간 여유가 있으므로 → **주문 넣고 자도 되는지** 명시"""
        ...
        market_focus = f"""
⑥ 이 브리핑은 【데일리 리뷰】입니다 (KST 07:00).
**데일리 리뷰는 투자 결정을 내리는 브리핑이 아니라, 전날의 결과와 현재 포지션 상태를 정리해서 다음 의사결정 브리핑(08:50 KR_OPEN)의 입력값을 만드는 브리핑이다.**

⑦ ★★ 절대 규칙 — 신규 거래 지시/권고 전면 금지 ★★
   이 브리핑에서는 **미래에 실행할 매수/매도/예약/조치를 일절 출력하지 않는다.** 구조화 필드(actions·strategy_buy·strategy_sell·night_orders)뿐 아니라 **본문 자연어에도** 아래 표현을 쓰지 마라:
   · 금지: "매수하세요/매수 권고/매수 추천/신규 진입/추가매수", "매도하세요/매도 권고/매도 추천", "비중 확대/비중 축소", "익절(하라)/손절(하라)/청산", "예약 주문", "오늘 실행/09:00 전 실행/오늘의 액션/매수 후보/매도 후보"
   · 허용(과거 복기): "어제 매수한 종목/어제 매도한 거래/어제 손절한 판단/어제 익절한 이유" — 지난 거래를 돌아보는 과거형은 OK.
   → 핵심 구분: **미래 실행 지시는 금지, 과거 거래 복기는 허용.**
   → 후속이 필요한 사안은 실행 지시가 아니라 **"KR_OPEN에서 재확인할 관찰 포인트"**로만 정리한다 (예: "한화에어로 과매도 지속 — KR_OPEN에서 진입 여부 재확인 요망").

⑧ 출력 구조 (반드시 이 3부, 이 섹션명만 사용):
   📊 어제 시장 결산 — 한국장 / 미국장 / 환율·금리·원자재 / 주요 이벤트
   📒 내 거래 복기 — 어제 실제 체결 내역 / 실현손익 / 판단이 맞았던 부분 / 아쉬웠던 부분 / 반복하면 안 되는 실수
   💼 현재 포지션 진단 — 보유 종목별 상태 / 수익률·손실률 / 리스크 요인 / KR_OPEN에서 재확인할 관찰 포인트
   ※ "오늘의 액션", "09:00 전 실행할 것", "매수/매도 후보", "예약 주문" 같은 섹션명은 절대 쓰지 마라.
{ria_focus}

━━━ 🌅 데일리 리뷰 규칙 ━━━
- actions = [] 고정. investment_decision은 "관망". strategy_buy/strategy_sell = []. night_orders = [].
- 어제 평가액 변화를 수치로: "전체 평가 N만 → M만 (±X만, ±Y%)"가 가능하면 제시.
- 내 거래 복기는 비판 포함 직언 — 아부 금지. 단 결과론적 비난 금지 (당시 정보 기준 판단의 적절성 평가).
- 포지션 진단은 시계 규칙: 〔장기〕는 논지 유효성만, 〔단기/중기〕는 "손익 상태 + 다음 분기점이 무엇인지" **진단만** (실행 지시 금지).
- 긴급 사안도 "KR_OPEN에서 확인 요망"으로만 넘긴다 — 여기서 실행시키지 않는다.
- 이 브리핑의 가치는 '복기를 통한 학습'과 '내 자산 현황 파악'이다. 신규 거래 아이디어는 다른 브리핑의 몫."""
    else:
        market_focus = ""
...
━━━ 【최우선 규칙 — 수입 중심 브리핑】 ━━━
1. 매수/매도 건수가 아니라 실제 수입/P&L 관리가 목적이다.
2. 보유관리·현금·리밸런싱을 신규 매수보다 먼저 다룬다.
3. 실현수입 / 오늘 평가변동 / 예상수입을 혼합하지 마라. 실현수입 원천이 없으면 "산출불가".
4. 양수 기대값(expected PnL) 후보만 추천한다.
5. 모든 BUY는 exit plan(목표/손절)과 thesis/red line이 필요하다.
6. Toss 자동 / 삼성 수동을 완전히 분리한다. Toss 주문표를 만들거나 복제하지 마라.
7. 구조화 income 수치(수입 계기판)를 재계산하거나 창작하지 마라.
...
- **확신도 규칙 (50~55 도피 금지)**: 과거 기록 분석 결과 확신도가 50대에 87% 몰려
...
- **눌림목 예약 매수는 예외**: 발굴/Watchlist 종목이 이미 급등(추격 불가)이지만 펀더멘털 양호(저PER 또는 매출성장+)면,
  현재가 즉시매수 대신 **눌림목 지정가로 strategy_buy에 등재**하라. 이때 "조건 2개 즉시 충족"을 요구하지 않는다 —
  진입가(눌림목가)에 도달했을 때 충족될 것을 전제로 예약. 단 진입가·손절가·무효화조건은 필수.
  · entry_price = 눌림목 지정가(예: 직전 급등의 38~50% 되돌림 또는 20일선 부근), execution_condition = "지정가 도달 시", strategy_type = "신규진입".
- 분할 매수 원칙: 1회 매수 금액은 해당 계좌 예수금의 30% 이내.
- **포지션 사이징은 비율 병기 필수**: "N주 (₩X, 계좌 예수금의 Y%)" 형식.
  사용자가 투자 원금을 증액할 계획이므로 절대 금액보다 % 기준이 재현 가능한 전략.
- **목표가는 horizon_days 내 도달 가능해야 한다** (과거 기록 46%가 무승부로 끝남 — 목표가 과도):
  · 단기(7일): 목표가는 현재가 대비 ±3~8% 범위 — ATR(일변동폭)×5를 넘는 목표 금지.
  · 목표가가 멀면 horizon_days를 14/21로 늘려서 정합시켜라. "7일 + 20% 목표" 조합 금지.
  · 추천은 검증 가능한 베팅이어야 한다 — 미지근한 의견이면 추천 말고 관망 + 트리거 제시.
- Watchlist 신규 종목도 동일 기준 적용 — 진입가/손절/익절 구체화.
- **매 브리핑마다 strategy_buy에 신규 진입 후보 최소 1개를 등재**할 것 (발굴/Watchlist/RIA ETF에서).
  · 발굴 종목(시장 스캐너/전시장 발굴)이 제공된 경우, 펀더멘털 양호 발굴주 중 **최소 1개를 strategy_buy에 반드시 등재**하라.
    즉시매수 조건 미충족이어도 **"눌림목 예약(신규진입)" 형태로는 반드시 등재** — 진입가(눌림목)/손절/무효화조건 포함.
  · "검토할 신규 후보 없음"은 금지. 발굴주를 그냥 관망하고 버리지 마라 — 눌림목 예약으로 살려라.
  · 모든 strategy_buy/strategy_sell row에 **account 필드 필수** ([일반]/[ISA]/[RIA]/[IRP]/[연금저축]). 누락 금지.
- 매수 추천은 구분 표시:
  · "추가매수" — 이미 보유 종목 추가 매수
...
- **매수/매도 균형 규칙**: strategy_sell에 N건을 낼 때 strategy_buy에도 신규 진입 후보(눌림목 예약 포함)를 최소 1건 제시하라.
  매도만 있고 매수 후보가 0건인 브리핑은 '관망 편향'으로 간주 — 발굴/Watchlist에서 후보를 반드시 끌어내라.
  (지난 30일 매도 35 : 매수 10의 편향이 발생했음 — 매수 기회를 놓치지 마라.)

【D-0. ⚡ actions — 브리핑의 최우선 출력 (사용자는 이것만 보고 실행)】
- "매수 추천/매도 추천/예약 주문"을 분리하지 마라 — 사용자가 실행할 모든 것을 actions 배열 하나에 통합.
- type은 4개로 고정:
  · "매수·즉시" — 지금 장에서 바로 주문 입력
  · "매도·즉시" — 지금 바로 매도
  · "예약매수" — 지정가 걸어두고 대기 (장 마감 후 브리핑이면 다음 장 예약)
  · "예약매도" — 익절/손절 예약 주문 (GTC 포함)
- 모든 action 필수 필드: 계좌, 종목, horizon〔장기/중기/단기〕, 주문방식, **단일 가격(범위 금지)**,
  수량(₩금액 + 예수금 %), 유효기간(당일/GTC), 목표가(+% 및 도달 시 행동), 손절가(-% 및 이탈 시 행동), 취소조건.
- "검토/모니터링/관망" 항목은 actions에 넣지 마라 — next_action의 대기 트리거로 분리.
- 실행할 것이 없으면 actions는 빈 배열 [] + next_action에 "오늘 실행할 액션 없음 + 대기 트리거" 명시.
- strategy_buy/strategy_sell은 분석 기록용으로 계속 채우되, actions와 내용이 일치해야 한다.
- 야간/프리브리핑의 night_orders도 actions의 예약매수/예약매도와 동일 내용이어야 한다 (actions가 마스터).

【D-1. 브리핑 출력 품질 게이트 — 계좌/실행성 분리】
- 브리핑은 반드시 계좌/범위별로 분리한다: 토스 AI 계좌 후보 / 삼성·수동 후보 / RIA·ISA·IRP 보유관리 / 신규 발굴 / 보유 리스크.
- /api/toss/buy-candidates의 items=[]는 "전체 살 후보 없음"이 아니다. 반드시 "토스 AI 계좌 후보 없음"으로 표현하고, 삼성/RIA/ISA/IRP 후보와 별도 판단하라.
- Toss AI 후보는 신규 발굴 전용이며, 삼성/RIA/ISA/IRP 브리핑 추천을 토스 후보로 재사용하지 마라.
- 모든 실행형/예약형 주문표는 필수 필드를 갖춰야 한다: 계좌, 종목명(티커), 방향, 현재가, 지정가/기준가, 수량, 총액, 현재가 대비 괴리율, 체결 가능성, 손절/무효화, 중복/한도, Hermes PASS/HOLD/BLOCK.
- 현재가=지정가 또는 지정가가 현재가보다 높으면 즉시체결 위험을 상단에 표시하고, PASS 전에는 HOLD/BLOCK으로 둔다.
- 누락 필드가 있으면 실행 후보로 표시하지 말고 "주문 차단·정보 부족" 또는 HOLD/BLOCK으로 분리한다.
- 신뢰도 보정은 평가 표본 기준으로만 한다. LMT/069500.KS/091160.KS는 고신뢰 가중, 161510.KS는 위험 감점. NVDA/GOOGL/PLTR/MSFT 등 표본부족 0%는 위험 경고 근거로 쓰지 마라.
- MU 같은 보호 보유종목은 "보유 관리 · 실행 매도 아님"으로 표시한다. 실행 매도처럼 쓰려면 별도 하드 무효화 조건과 사용자 승인 필요.
...
- [토스 AI]: 삼성과 분리된 제한형 완전자율 실계좌. Toss 후보와 주문 상태는 income_briefing 구조화 데이터만 신뢰한다. LLM은 Toss 주문을 새로 만들거나 수동 주문표로 복제하지 않는다.
...
{{
  ...
  "actions": [
    {{
      "type": "매수·즉시|매도·즉시|예약매수|예약매도",
      "account": "[ISA]|[일반]|[RIA]|[IRP]|[연금저축]",
      "ticker": "종목코드", "name": "종목명",
      "horizon": "장기|중기|단기",
      "order_method": "지정가|시장가",
      "price": "₩40,500 (단일가, 범위 금지)",
      "qty": "15주 (₩61만, 예수금 14%)",
      "validity": "당일|GTC|조건부",
      "target": "₩43,500 (+7.4%) 도달 시 전량 매도",
      "stop": "₩38,800 (-4.2%) 이탈 시 즉시 매도",
      "cancel_if": "갭업 +1.5% 초과 시 미실행 등 취소조건",
      "long_term_plan": "장기일 때만: 적립 계획 + 보유 기간 + 무효화 조건. 그 외 빈 문자열",
      "reason": "한 줄 근거"
    }}
  ],
  "portfolio_rows": [
  ...
  "strategy_buy": [
    {{
      ...
      "strategy_type": "신규진입|장기적립|단기매매|중기보유|리밸런싱|세금전략|일반",
