...
[자동 BUY 게이트 — avoid_only 1단계]
evaluate_ai_berkshire_buy_gate()는 SELL 게이트와 반대 방향이다. 기존 운영을
갑자기 0건으로 만들지 않기 위해, 근거가 살아있는(freshness_valid) effective
avoid 판정만 신규 BUY를 하드 차단한다. unscored / expired / invalid /
gray_zone은 차단하지 않고 research_status 진단만 남겨 재리서치 큐로 넘긴다.
hold/protect/trim/sell_to_fund는 BUY 의미를 자동 결정하지 않는다
(reviewed_non_avoid).
"""
...
# 자동 신규 BUY 하드 차단 판정 (avoid_only 1단계 — 그 외는 진단만)
